...
inferencer = DetInferencer(model_name, GLIP_CHECKPOINT_PATH, device)
...
def getPredictions(promptValue, imagePath):
...
def getFinalPredictions(images,imagePaths, promptVals):
  masks = []
  emptyPrompt = ""
  for img, image_path in (pbar := tqdm(zip(images,imagePaths))):
    imageMasks = {}
...
def main():
    ...
    for i in range(num_slice):
        ...
        images, truthMasks, data_list,imagePaths = getImages2(start,end)
        ...
        masks = getFinalPredictions(images,imagePaths, bodyPrompts)
    ...
    miou_table.to_csv('glip_sam_result/miou.csv')

    pix_acc_table = pd.DataFrame(pix_acc_table, columns=pix_acc_table[0].keys()).set_index('name')
    pix_acc_table.to_csv('glip_sam_result/pix_acc.csv')
